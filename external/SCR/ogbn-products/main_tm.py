    ...
    for stage, epochs in enumerate(args.stages):
        if stage > 0 and args.use_rlu:
            ...
            if len(extra_confident_nid) > 0:
                enhance_loader = torch.utils.data.DataLoader( 
                        enhance_idx, batch_size=int(args.batch_size*len(enhance_idx)/(len(enhance_idx)+train_node_nums)), shuffle=True, drop_last=False) # TM temporary edit 11/14/25
        ...
        if args.giant and args.method == "SAGN":
            ...
            print("Best Epoch {}, Val {:.4f}, Test {:.4f}".format(
                best_epoch, best_val, best_test))
    # TM edited this block for saving the model 11/14/25
    model_id = f"{args.dataset}_{args.method}_hid{args.hidden}_stg{len(args.stages)}_run{args.seed}"
    model_path = f"{dirs}/model_{model_id}.pth"
    torch.save(best_model.state_dict(), model_path)
    return best_val, best_test, preds # TM edited 11/14/25
...
def main(args):
    ...
    for i in range(args.num_runs):
        ...
        best_val, best_test, preds = run(args, device) # TM edited 11/14/25
        np.save(f"output/{args.dataset}/output_preds_{i}.npy", preds.numpy()) # TM edited 11/14/25
